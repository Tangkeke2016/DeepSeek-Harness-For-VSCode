...
export function followEditorTheme(): () => void {
  ...
  /** Read VS Code's light/dark decision off the Webview body classes. */
  ...
  /** One proxied `prefers-color-scheme` query that re-emits when VS Code's theme flips. */
  class EditorQuery extends EventTarget implements MediaQueryList {
    ...
    get matches(): boolean {
      const value = dark();
      return value === undefined ? this.native.matches : value === this.wantsDark;
    }
    ...
    dispose(): void {
      this.native.removeEventListener('change', this.sync);
    }
  }
  ...
  const matchMedia: typeof window.matchMedia = query => {
    ...
    // One cached query per preference keeps listeners attached across calls.
    let result = queries.get(color[1]!);
    if (!result) {
      result = new EditorQuery(original.call(window, query), color[1] === 'dark');
      queries.set(color[1]!, result);
    }
    return result;
  };
  ...
  const observer = new MutationObserver(() => {
    // The client may replace the body element while it boots.
  ...
  const observe = (): void => {
    observer.disconnect();
    body = document.body;
    // Watch the body's theme classes once it exists, otherwise wait for it.
  ...
  return () => {
    ...
    if (window.matchMedia === matchMedia) window.matchMedia = original;
  };
}
