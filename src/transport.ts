...
export class Transport {
  ...
  async connect(input: string): Promise<void> {
    ...
    // The launch URL redirects once and sets the cookie every later call reuses.
    ...
    // One multiplexed socket carries every Remote stream belonging to this view.
    ...
    socket.on('message', (bytes, binary) => {
      try {
        ...
        if (frame.type === 'item') stream.item(frame.value);
          ...
          // A terminal frame fails the stream and carries the remote failure with it.
    ...
    // Resolve on the handshake, or reject when the socket dies before opening.
  ...
  async call(endpoint: string, args: Record<string, unknown>): Promise<unknown> {
    ...
    // The server correlates the answer through this id, so a mismatched or
    // foreign response is rejected rather than returned to the caller.
  ...
  async request(path: string, init: RequestInit = {}): Promise<Response> {
    ...
    // Credentials follow the authenticated origin only; a redirect never carries them.
  ...
  open(endpoint: string, args: Record<string, unknown>, item: Stream['item'], fail: Stream['fail']): () => void {
    ...
    // The server keys this stream by id; the returned canceller frees it.
  ...
  private fail(error: Error): void {
    ...
    // Fail every open stream before reporting the connection loss once.
