...
export class MessageDelivery {
  ...
  dispose(): void { this.disposed = true; this.queue.length = 0; this.bytes = 0; this.acknowledgement?.finish(new Error('View disposed')); }
  private fail(error: Error): void { if (this.disposed) return; this.dispose(); this.failed(error); }
