...
export class WebRelay {
  ...
  private async handle(input: unknown): Promise<boolean> {
    ...
    try {
      if (kind === 'stream-open') {
        ...
        this.streams.set(id, cancel); return true;
      }
      ...
      try {
        ...
        if (kind === 'bundle') {
          ...
          for (const entry of array(message.headers)) {
            const pair = array(entry); const name = string(pair[0]); const value = string(pair[1]);
  ...
  private emit(value: unknown): void { if (!this.disposed) this.send(value); }
