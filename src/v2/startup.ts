...
const zh: typeof en = { tagline: '探索未至之境', checking: '正在检查运行环境…', loading: '正在加载 Harness…', node: 'Node.js 未找到或版本不满足要求。请安装 Node.js 22.19+（22.x）或 24+，加入 PATH 后重启 VS Code。SSH 工作区请在远程机器安装。', download: '前往 Node.js 官方下载', bin: '未找到官方 DSH ({project})的 bin.js，请选择 bin.js 文件或已安装的 DSH 目录。', binAction: '选择 bin 文件', directoryAction: '选择 DSH 目录', binPrompt: '选择官方 DSH 的 bin.js 文件', directoryPrompt: '选择已安装依赖并构建完成的 DSH 目录', project: 'DSH 官方项目', retry: '重新加载', invalid: '请选择有效的 DSH bin.js 文件或安装目录。', saved: 'DSH 路径已保存到当前运行主机的 VS Code 用户设置。' };
...
${state.binError !== undefined ? `<section><strong>DSH bin.js</strong><p>${escape(t.bin).replace('{project}', `<a href="https://github.com/deepseek-ai/deepseek-harness" id="dsh-project">${escape(t.project)}</a>`)}</p>${button('setup-bin', t.binAction)}${button('setup-directory', t.directoryAction)}</section>` : ''}
</main><script nonce="${nonce}">const api=acquireVsCodeApi();for(const b of document.querySelectorAll('[data-action]'))b.onclick=()=>api.postMessage({kind:b.dataset.action});</script></body></html>`;
}
