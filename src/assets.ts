...
export class ClientAssets {
  ...
  async fetch(path: string, fetcher: (path: string) => Promise<Response>): Promise<Response> {
    // Only content-addressed resources are immutable enough to share between views.
    ...
    // Buffer the body so one size limit covers the whole resource.
    ...
    // Hop-by-hop and credential headers must not be replayed to the Webview.
    ...
    if (bytes.byteLength <= this.limit) {
      ...
      if (previous) { this.size -= previous.bytes.byteLength; this.entries.delete(path); }
      // Evict in insertion order until this resource fits the budget.
      while (this.size + bytes.byteLength > this.limit) {
        const first = this.entries.keys().next().value!;
        this.size -= this.entries.get(first)!.bytes.byteLength;
        this.entries.delete(first);
      }
      this.entries.set(path, { bytes: bytes.buffer, headers });
