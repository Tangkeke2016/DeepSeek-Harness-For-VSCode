  ...
  private async connect(options: BackendOptions): Promise<string> {
    ...
    while (Date.now() < deadline) {
      ...
      if (status) {
        ...
        if (status.state === 'ready' && status.url) { this.log(`Shared backend ${spawned ? 'connected' : 'reused'} (${Date.now() - (deadline - options.startupTimeoutSeconds * 1000)} ms)`); return status.url; }
